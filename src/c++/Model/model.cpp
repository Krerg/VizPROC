...
void Model::start()
{
    ...
    if(diodes->size()<=0) {
        ...
        qDebug()<<"Graph close stage completed";
        ...
        while(true) {
            ...
            calculationResult = calculate();
            qDebug()<<"Before open next branch";
            if(!graph->openNextBranch(calculationResult)) {
                break;
            }
        }
    ...
    graph->optimizeGraph();
    qDebug()<<"All calculations completed";
