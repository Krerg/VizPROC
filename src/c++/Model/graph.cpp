...
QList<Branch *> *Graph::getConnectedBranches(int vertexNumber)
{
    ...
    for(int i=0;i<matrixSize;i++) {
        if(adjacencyMatrix[vertexNumber][i]!=NULL && adjacencyMatrix[vertexNumber][i]->size()>0) {
...
{
    qDebug()<<"Null safe adding direct edge";
    if(vertexNumber2>vertexNumber1) {
        nullSafeAddDirectEdge(branch,vertexNumber2,vertexNumber1);
        return;
    }
    qDebug()<<vertexNumber1<<"v1";
    qDebug()<<vertexNumber2<<"v2";
    if(adjacencyMatrix[vertexNumber1][vertexNumber2]==NULL) {
        qDebug()<<"C1";
...
{
    qDebug()<<"Clearing allocated Array";
    if(matrixSize<=0) {
        return;
    }
...
bool Graph::chackDiodeBranchesDirection(Branch *branch1, Branch *branch2)
{
    if(branch1!=NULL && branch2!=NULL) {
        if((branch1->getVertexNumber1()!=branch2->getVertexNumber1())
                && (branch1->getVertexNumber2()!=branch2->getVertexNumber2())) {
            return true;
        }
    }
    return false;
...
void Graph::unionEdges(int centralVertexNumber, int firstConnectedVertex, int secondConnectedVertex)
{
    ...
    if(firstConnectedBranch->isDiodeBranch()) {
        ...
        this->diodeBranches->removeAll(secondConnectedBranch);
    } else if(firstConnectedBranch->isDiodeBranch() && secondConnectedBranch->isDiodeBranch()) {
        //TODO проверить
        firstConnectedBranch->unionBranch(secondConnectedBranch);
        unionBranch=firstConnectedBranch;
    } else {
    ...
    if((firstConnectedBranch->isDiodeBranch() || firstConnectedBranch->isHasEmf())
            && (secondConnectedBranch->isDiodeBranch() || secondConnectedBranch->isHasEmf())) {
        if(chackDiodeBranchesDirection(firstConnectedBranch,secondConnectedBranch)) {

        } else {
            //TODO throw an exception
        }
    } else if(firstConnectedBranch->isDiodeBranch() || firstConnectedBranch->isHasEmf()) {
...
bool Graph::checkDiodeBranchTreshold(double *calcResult, Branch *diodeBranch)
{
    ...
    if(diodeBranch->isHasEmf()) {
        foreach (Element* elem, *diodeBranch->getBranchElements()) {
            if(elem->getName()==StringValues::EMF_NAME) {
                if(((EMF*)elem)->getVoltage()>diodeBranch->getDiodeCount()*ConstValues::DIODE_OPEN_TRESHOLD) {
                    return true;
                } else {
                    return false;
                }
    ...
    if((upPotential-downPotential)>diodeBranch->getDiodeCount()*ConstValues::DIODE_OPEN_TRESHOLD) {
        return true;
    } else {
        return false;
    }
